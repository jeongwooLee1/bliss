...
function App() {
  ...
  // Save current page to session whenever it changes (only after app loaded)
  useEffect(() => {
    if (phase !== "app") return;
    try{const s=JSON.parse(sessionStorage.getItem("bliss_session")||"{}");s.page=page;sessionStorage.setItem("bliss_session",JSON.stringify(s));}catch{}
  }, [page, phase]);
        ...
        if (users.length === 0) {
          ...
          try {
            ...
            if (saved?.userId) {
              ...
              if (u) {
                if (saved.bizId) window._pendingBizId = saved.bizId;
                if (saved.page) window._pendingPage = saved.page;
                console.log("Auto-login:", u.role, "bizId:", saved.bizId, "page:", saved.page);
  ...
  const handleLogin = async (user, isAutoLogin) => {
    ...
    if (!isAutoLogin) {
      try{sessionStorage.setItem("bliss_session",JSON.stringify({userId:user.id,loginId:user.loginId||user.login_id}));}catch{}
    }
    if (user.role === "super") {
      ...
      try {
        ...
        const pendingBiz = window._pendingBizId;
        delete window._pendingBizId;
        if (pendingBiz) { setCurrentUser(user); setRole(user.role); setSuperData(sd); handleEnterBiz(pendingBiz); return; }
        setPhase("super");
      } catch(e) {
        console.error(e);
      ...
      try {
        ...
        // Restore saved page
        const pp = window._pendingPage; delete window._pendingPage; console.log("Restoring page:", pp);
        if (pp && ["timeline","reservations","sales","stats","customers","users","admin"].includes(pp)) setPage(pp);
        setPhase("app");
      } catch(e) {
  ...
  const handleEnterBiz = async (bizId) => {
    try{const s=JSON.parse(sessionStorage.getItem("bliss_session")||"{}");s.bizId=bizId;sessionStorage.setItem("bliss_session",JSON.stringify(s));}catch{}
    setLoadMsg("매장 데이터 로딩 중...");
    setPhase("loading");
    try {
      const bizList = await sb.get("businesses", `&id=eq.${bizId}`);
  ...
  const handleLogout = () => {
    try{sessionStorage.removeItem("bliss_session");}catch{}
  ...
  const handleBackToSuper = async () => {
    try{const s=JSON.parse(sessionStorage.getItem("bliss_session")||"{}");delete s.bizId;sessionStorage.setItem("bliss_session",JSON.stringify(s));}catch{}
    setLoadMsg("관리자 데이터 로딩 중...");
    setPhase("loading");
    const sd = await loadAllFromDb(null);
    setSuperData(sd);
...
function Timeline({ data, setData, userBranches, viewBranches=[], isMaster, currentUser, setPage, bizId }) {
  ...
  const [blockFs, setBlockFs] = useState(10); // 블록 텍스트 크기
  const [blockOp, setBlockOp] = useState(100); // 블록 투명도 %
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
                      ...
                      <div key={block.id}
                        ...
                        {block.type==="reservation" && !block.isSchedule && <>
                          <div style={{display:"flex",alignItems:"center",gap:3,overflow:"hidden",whiteSpace:"nowrap"}}>
                            ...
                            <span style={{fontWeight:600,color:isNaverCancelled?"#999":"#333",overflow:"hidden",textOverflow:"ellipsis",textDecoration:isNaverCancelled?"line-through":"none"}}><span style={{color:block.custGender==="M"?"#4a7cc8":"#e57373"}}>{block.custGender==="M"?"남":"여"}</span> {block.custName}</span>
                          </div>
      ...
      {dragBlock && dragPos && <div style={{position:"fixed",
        ...
        <div style={{fontWeight:500,fontSize:9,opacity:.85}}><span style={{color:dragBlock.custGender==="M"?"#4a7cc8":"#e57373"}}>{dragBlock.custGender==="M"?"남":"여"}</span> {dragBlock.custName}</div>
...
function DetailedSaleForm({ reservation, branchId, onSubmit, onClose, data, setData }) {
  ...
  // 네이버 예약 감지
  const isNaver = !!(reservation?.selectedTags||[]).some(tid => {
    const tag = (data?.serviceTags||[]).find(t=>t.id===tid);
    return tag && tag.name.includes("네이버");
  });
  const [naverPrepaid, setNaverPrepaid] = useState(0);
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: "10px 16px", borderTop: "1px solid #e0e0e0", display: "flex", gap: 10, alignItems: "center", justifyContent: "space-between", background: "#f8f8f8" }}>
          <div style={{ fontSize: 11, color: "#d0d0d0" }}>
            {gender === "F" ? "여성" : "남성"} 가격 적용 · 체크한 항목만 매출 반영 · 관리설정에서 시술/제품 편집 가능
          </div>
        ...
        {res.length===0?<tr><td colSpan={10} style={{textAlign:"center",color:"#999",padding:40}}>예약이 없습니다</td></tr>:
          res.map(r=>{
            ...
            return <tr key={r.id}>
              ...
              <td><span className="badge" style={{background:g==="M"?"#e0edf5":"#fce8e8",color:g==="M"?"#7c7cc8":"#e57373",fontSize:10}}>{g==="M"?"남":"여"}</span></td>
        ...
        {sales.length===0?<tr><td colSpan={21} style={{textAlign:"center",color:"#999",padding:40}}>매출 기록 없음</td></tr>:
          sales.map((s,i)=>{
            ...
            return <tr key={s.id}>
              ...
              <td>{s.custNum}</td>
              <td style={{fontWeight:600}}><span style={{color:s.custGender==="M"?"#4a7cc8":"#e57373",marginRight:3}}>{s.custGender==="M"?"남":"여"}</span>{s.custName||"-"}</td>
