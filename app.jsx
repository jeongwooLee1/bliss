    ...
    <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
          ...
          {(() => {
            ...
            return <div style={{position:"relative",flexShrink:0}}>
              <button onClick={()=>setShowBranchPicker(p=>!p)}
                style={{height:28,padding:"0 8px",fontSize:10,border:"1px solid #d0d0d0",borderRadius:4,
        ...
        return <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:9998,background:"rgba(0,0,0,0.3)",
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,boxShadow:"0 12px 40px rgba(0,0,0,0.2)",padding:"20px 24px",
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              <button onClick={cancelChange} style={{flex:1,padding:"10px 0",fontSize:13,fontWeight:600,borderRadius:8,border:"1px solid #ddd",background:"#f5f5f5",color:"#666",cursor:"pointer",fontFamily:"inherit"}}>취소</button>
      ...
      {showSettings && <>{(() => {
        ...
        <div
          ...
          maxHeight:"80vh",overflowY:"auto",animation:"slideUp .6s cubic-bezier(.22,1,.36,1)",willChange:"transform"}}>
        ...
        {branches.map((b,i) => (
          <div key={b.id}
            ...
            style={{
              ...
              background:"#fff",
              boxShadow:dragOverIdx===i?"0 0 0 2px #7c7cc8":"0 1px 3px rgba(0,0,0,.06), 0 1px 2px rgba(0,0,0,.04)",
              opacity:dragIdx===i?0.4:1, touchAction:"none",
              transition:"box-shadow .2s"
            }}>
            {/* Header bar */}
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",
              background:`linear-gradient(135deg, ${b.color||"#f0f0f0"}18, ${b.color||"#f0f0f0"}08)`,
              ...
              <I name="grip" size={12} color="#ccc"/>
              <div style={{width:8,height:8,borderRadius:"50%",background:b.color||"#ddd",flexShrink:0}}/>
              <span style={{flex:1,fontSize:14,fontWeight:700,color:"#222"}}>{b.name||"새 지점"}</span>
              <span style={{fontSize:10,padding:"2px 8px",borderRadius:10,fontWeight:600,
                background:b.useYn!==false?"#34c75920":"#ff3b3020",
                color:b.useYn!==false?"#34c759":"#ff3b30"}}>{b.useYn!==false?"사용":"미사용"}</span>
            </div>
            {/* Form rows - iOS Settings style */}
            <div style={{fontSize:13}}>
              {[
                {label:"지점명", val:b.name, field:"name", ph:"지점명"},
                ...
                <div key={row.field} className="admin-place-row" style={{display:"flex",alignItems:"center",padding:"0 14px",
                  ...
                  <input value={row.val} onChange={e=>updateField(b.id,row.field,e.target.value)}
                    placeholder={row.ph} style={{flex:1,fontSize:13,border:"none",background:"transparent",padding:"10px 0",borderRadius:0,outline:"none",fontFamily:"inherit",color:"#333",WebkitTapHighlightColor:"transparent"}}/>
                </div>
              ))}
              {/* 사용구분 */}
              <div className="admin-place-row" style={{display:"flex",alignItems:"center",padding:"0 14px",borderBottom:"1px solid #f2f2f7",minHeight:44}}>
                ...
                <select value={b.useYn!==false?"Y":"N"} onChange={e=>updateField(b.id,"useYn",e.target.value==="Y")}
                  style={{flex:1,fontSize:13,appearance:"none",border:"none",background:"transparent",padding:"10px 0",borderRadius:0,outline:"none",fontFamily:"inherit",WebkitTapHighlightColor:"transparent",
                    color:b.useYn!==false?"#34c759":"#ff3b30",fontWeight:600}}>
                  <option value="Y">사용</option><option value="N">미사용</option>
                </select>
              </div>
              {/* 색상 */}
              <div style={{display:"flex",alignItems:"center",padding:"8px 14px",borderBottom:"1px solid #f2f2f7",minHeight:44}}>
                <span style={{width:72,fontSize:12,color:"#666",flexShrink:0,fontWeight:500}}>색상</span>
                <div style={{flex:1,display:"flex",alignItems:"center",gap:8}}>
                  ...
                  <input value={b.color||""} onChange={e=>updateField(b.id,"color",e.target.value)}
                    style={{width:80,border:"1px solid #e8e8e8",background:"#f8f8f8",fontSize:11,fontFamily:"monospace",
                      padding:"4px 8px",borderRadius:8,color:"#555",outline:"none"}}/>
                </div>
              </div>
              {/* 네이버 연동 섹션 */}
              <div style={{padding:"8px 14px 10px",background:"#fafbfc"}}>
                <div style={{fontSize:10,color:"#03c75a",fontWeight:700,marginBottom:6,display:"flex",alignItems:"center",gap:4}}>
                  <span style={{width:6,height:6,borderRadius:"50%",background:"#03c75a",display:"inline-block"}}/>네이버 연동
                </div>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  <input value={b.naverEmail||""} onChange={e=>updateField(b.id,"naverEmail",e.target.value)}
                    placeholder="Gmail" style={{flex:"1 1 120px",background:"#fff",border:"1px solid #e8e8e8",fontSize:12,padding:"8px 10px",borderRadius:8,outline:"none",fontFamily:"inherit"}}/>
                  <input value={b.naverBizId||""} onChange={e=>updateField(b.id,"naverBizId",e.target.value)}
                    placeholder="Biz ID" style={{width:80,background:"#fff",border:"1px solid #e8e8e8",fontSize:12,padding:"8px 10px",borderRadius:8,outline:"none",fontFamily:"inherit"}}/>
                  {b.naverEmail && <input type="number" value={b.naverColCount||1} min={1} max={5}
                    onChange={e=>updateField(b.id,"naverColCount",Math.max(1,Math.min(5,parseInt(e.target.value)||1)))}
                    style={{width:44,background:"#fff",border:"1px solid #e8e8e8",fontSize:12,padding:"8px 4px",borderRadius:8,textAlign:"center",outline:"none",fontFamily:"inherit"}}/>}
                </div>
              </div>
            </div>
            {/* Footer */}
            <div style={{display:"flex",gap:8,padding:"8px 14px",background:"#fafafa",justifyContent:"flex-end",alignItems:"center"}}>
              {deleteId===b.id ? <>
                <button onClick={()=>deleteBranch(b.id)} style={{padding:"7px 16px",fontSize:12,fontWeight:700,borderRadius:8,border:"none",background:"#ff3b30",color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>삭제 확인</button>
                <button onClick={()=>setDeleteId(null)} style={{padding:"7px 14px",fontSize:12,borderRadius:8,border:"1px solid #d0d0d0",background:"#fff",color:"#888",cursor:"pointer",fontFamily:"inherit"}}>취소</button>
              </> : <>
                <button onClick={()=>setDeleteId(b.id)} style={{padding:"7px 10px",fontSize:12,borderRadius:8,border:"none",background:"transparent",color:"#ff3b30",cursor:"pointer",fontFamily:"inherit"}}><I name="trash" size={12}/></button>
                {edited[b.id] && <button className="btn-p" onClick={()=>applyRow(b.id)}
                  style={{padding:"7px 20px",fontSize:12,fontWeight:700,borderRadius:8,background:"#7c7cc8"}}>저장</button>}
              </>}
            </div>
          </div>
        ))}
