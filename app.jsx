...
const STATUS_LABEL = { confirmed:"진행중", completed:"완료", cancelled:"취소", no_show:"노쇼", pending:"확정대기", naver_cancelled:"네이버취소" };
const STATUS_CLR = { confirmed:"#FF9800", completed:"#6bab9e", cancelled:"#e57373", no_show:"#ef5350", pending:"#FF9800", naver_cancelled:"#E6A700" };
  ...
  return (
    <div className="ov" onClick={onClose} style={{alignItems:"flex-start",paddingTop:20,overflow:"auto"}}>
      <div ref={modalRef} onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"8px",border:"1px solid #e0e0e0",
        ...
        <div style={{display:"flex",gap:6,padding:"12px 16px",borderTop:"1px solid #e0e0e0",background:"#fafafa",borderRadius:"0 0 8px 8px",flexShrink:0,flexWrap:"wrap",alignItems:"center"}}>
            {/* 상태 뱃지 */}
            {item?.id && !isSchedule && f.status && <span className="badge" style={{background:`${STATUS_CLR[f.status]||"#999"}20`,color:STATUS_CLR[f.status]||"#999",fontSize:11,padding:"4px 10px",fontWeight:700}}>{STATUS_LABEL[f.status]||f.status}</span>}
            ...
            {item?.id && !isSchedule && f.status !== "cancelled" && <button style={{padding:"10px 16px",borderRadius:8,border:"1px solid #e57373",background:"#fff",color:"#e57373",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit"}} onClick={()=>{
              if(!window.confirm("이 예약을 취소 처리하시겠습니까?")) return;
              const updated = {...f, status:"cancelled"};
              onSave(updated); onClose();
            }}><I name="x" size={12}/> 취소</button>}
