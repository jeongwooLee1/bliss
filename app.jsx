          ...
          {isSchedule && <>
            <div>
              ...
              <div className="res-time-row" style={{display:"flex",gap:3,alignItems:"center",flexWrap:"nowrap",overflowX:"auto"}}>
                ...
                <select className="inp" style={{flex:"0 0 78px",fontSize:12,padding:"5px 6px"}} value={f.time} onChange={e=>{set("time",e.target.value);set("dur",calcDur(e.target.value,f.endTime))}}>
                ...
                <select className="inp" style={{flex:"0 0 78px",fontSize:12,padding:"5px 6px"}} value={f.endTime} onChange={e=>{set("endTime",e.target.value);set("dur",calcDur(f.time,e.target.value))}}>
                ...
                <select className="inp" style={{flex:"1 1 0",minWidth:0,fontSize:12,padding:"5px 6px"}} value={`${f.roomId}|${f.staffId}`} onChange={e=>{const [r,s]=e.target.value.split("|");set("roomId",r);set("staffId",s)}}>
...
function DatePick({ value, onChange, style, min }) {
  ...
  const fmt = (v) => { if (!v) return "--"; const p = v.split("-"); return `${p[0].slice(2)}.${p[1]}.${p[2]}`; };
  return <div style={{position:"relative",display:"inline-flex",...style}}>
    <div className="inp" onClick={()=>ref.current?.showPicker?.()||ref.current?.click()}
      style={{width:"100%",fontSize:12,padding:"5px 6px",cursor:"pointer",textAlign:"center",whiteSpace:"nowrap"}}>{fmt(value)}</div>
