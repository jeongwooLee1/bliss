...
function App() {
  ...
  // ─── 예약 실시간 동기화 (Realtime + 폴링 fallback) ───
  const rtStatus = useRef("off");
  useEffect(() => {
    if (phase !== "app" || !currentBizId) return;
    let channel = null;
    let pollIv = null;
    const supaClient = typeof window !== "undefined" && window.supabase ? window.supabase.createClient(SB_URL, SB_KEY) : null;
    ...
    const poll = async () => {
      try {
        ...
        setData(prev => {
          if (!prev) return prev;
          const prevIds = new Set(prev.reservations.map(r => r.id + "|" + (r._ua||"")));
          const newIds = new Set(parsed.map(r => r.id + "|" + (r._ua||"")));
          let changed = prev.reservations.length !== parsed.length;
          if (!changed) { for (const id of prevIds) { if (!newIds.has(id)) { changed = true; break; } } }
          return changed ? {...prev, reservations: parsed} : prev;
        });
      } catch(e) { console.warn("Poll error:", e); }
    };

    // 폴링 시작 (항상 동작, Realtime 성공 시 간격 늘림)
    const startPoll = (interval) => { clearInterval(pollIv); pollIv = setInterval(poll, interval); };
    startPoll(20000); // 기본 20초

    // Realtime 시도
    if (supaClient) {
      try {
        channel = supaClient.channel("bliss-res-" + currentBizId)
          ...
          .subscribe((status, err) => {
            ...
            rtStatus.current = status;
            if (status === "SUBSCRIBED") {
              startPoll(60000); // Realtime 성공 → 폴링 60초로 완화
              console.log("✅ Realtime 연결 성공 (폴링 60초로 전환)");
            }
            if (status === "CLOSED" || status === "TIMED_OUT" || status === "CHANNEL_ERROR") {
              startPoll(20000); // Realtime 실패 → 폴링 20초 유지
              console.log("⚠️ Realtime 실패, 폴링 20초 유지");
            }
          });
...
function Timeline({ data, setData, userBranches, viewBranches=[], isMaster, currentUser, setPage, bizId }) {
  ...
  const setStatusClr = (k,v) => { setStatusClrRaw(p => { const n = {...p,[k]:v}; dbTl.current = {...dbTl.current, sc:n}; tlSaveDb(dbTl.current); try{localStorage.setItem("tl_sc",JSON.stringify(n))}catch(e){} return n; }); };
