  ...
  return <div>
    <div className={isMobileView?undefined:"card"} style={{padding:isMobileView?0:20}}>
      ...
      {expandedCard && <>{(() => {
        ...
        return <>
          <div style={{position:"fixed",inset:0,zIndex:999,background:"rgba(0,0,0,.4)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)"}}
            onClick={()=>setExpandedCard(null)}/>
          <div style={{position:"fixed",left:12,right:12,top:"8vh",zIndex:1000,
            background:"#fff",borderRadius:16,overflow:"hidden",
            boxShadow:"0 24px 80px rgba(0,0,0,.25)",
            animation:"cardOpen .35s cubic-bezier(.2,.9,.3,1)"}}>
            {/* Header */}
            <div style={{padding:"16px 18px",background:"linear-gradient(135deg,#7c7cc815,#7c7cc808)",
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{fontSize:17,fontWeight:800,color:"#222"}}>{b.name||"새 지점"}</span>
              </div>
              <button onClick={()=>setExpandedCard(null)}
                style={{width:28,height:28,borderRadius:"50%",background:"#f0f0f0",border:"none",
                  display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:14,color:"#999"}}>✕</button>
            </div>
            {/* Form */}
            <div style={{padding:"0",maxHeight:"70vh",overflowY:"auto"}}>
              {[
                ...
                <div key={row.field} style={{display:"flex",alignItems:"center",padding:"0 18px",borderBottom:"1px solid #f2f2f7",minHeight:48}}>
                  <span style={{width:72,fontSize:13,color:"#888",flexShrink:0,fontWeight:500}}>{row.label}</span>
              ...
              <div style={{borderBottom:"1px solid #f2f2f7"}}>
                <div style={{display:"flex",alignItems:"center",padding:"0 18px",minHeight:48}}>
                  <span style={{width:72,fontSize:13,color:"#888",flexShrink:0,fontWeight:500}}>메일주소</span>
                  <input value={b.naverEmail||""} onChange={e=>updateField(b.id,"naverEmail",e.target.value)}
                    placeholder="example@gmail.com" style={INP}/>
                </div>
                <div style={{padding:"0 18px 8px 90px",fontSize:11,color:"#aaa",lineHeight:1.4}}>
                  네이버 예약 알림을 수신할 Gmail 주소입니다. Apps Script 연동 시 이 메일로 들어오는 네이버 예약을 자동으로 타임라인에 반영합니다.
                </div>
              </div>
              {/* 네이버 Biz ID */}
              <div style={{display:"flex",alignItems:"center",padding:"0 18px",borderBottom:"1px solid #f2f2f7",minHeight:48}}>
                <span style={{width:72,fontSize:13,color:"#888",flexShrink:0,fontWeight:500}}>비즈 ID</span>
                <input value={b.naverBizId||""} onChange={e=>updateField(b.id,"naverBizId",e.target.value)}
                  placeholder="네이버 예약 비즈니스 ID (숫자)" style={INP}/>
              </div>
              {/* 칼럼 수 */}
              <div style={{display:"flex",alignItems:"center",padding:"0 18px",borderBottom:"1px solid #f2f2f7",minHeight:48}}>
                <span style={{width:72,fontSize:13,color:"#888",flexShrink:0,fontWeight:500}}>칼럼 수</span>
                <div style={{flex:1,display:"flex",alignItems:"center",gap:8,padding:"8px 0"}}>
                  {[1,2,3,4,5].map(n=>(
                    <button key={n} onClick={()=>updateField(b.id,"naverColCount",n)}
                      style={{width:36,height:36,borderRadius:10,border:"none",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit",
                        background:(b.naverColCount||1)===n?"#7c7cc8":"#f2f2f7",
                        color:(b.naverColCount||1)===n?"#fff":"#888"}}>{n}</button>
                  ))}
                  <span style={{fontSize:11,color:"#bbb",marginLeft:4}}>개</span>
                </div>
              </div>
            </div>
            {/* Footer */}
            <div style={{display:"flex",gap:8,padding:"12px 18px",background:"#fafafa",borderTop:"1px solid #f0f0f0"}}>
              {deleteId===b.id ? <>
                <button onClick={()=>{deleteBranch(b.id);setExpandedCard(null);}} style={{flex:1,padding:"12px",fontSize:14,fontWeight:700,borderRadius:10,border:"none",background:"#ff3b30",color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>삭제 확인</button>
                <button onClick={()=>setDeleteId(null)} style={{flex:1,padding:"12px",fontSize:14,borderRadius:10,border:"1px solid #d0d0d0",background:"#fff",color:"#888",cursor:"pointer",fontFamily:"inherit"}}>취소</button>
              </> : <>
                <button onClick={()=>setDeleteId(b.id)} style={{padding:"12px 16px",fontSize:13,borderRadius:10,border:"none",background:"#ff3b3010",color:"#ff3b30",cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>
                  <I name="trash" size={12}/> 삭제
                </button>
                <div style={{flex:1}}/>
                {edited[b.id] && <button onClick={()=>applyRow(b.id)}
                  style={{padding:"12px 28px",fontSize:14,fontWeight:700,borderRadius:10,border:"none",background:"#7c7cc8",color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>저장</button>}
              </>}
            </div>
          </div>
        </>;
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {branches.map((b,i) => (
          <div key={b.id} onClick={()=>setExpandedCard(b.id)}
            style={{
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes cardOpen{from{opacity:0;transform:scale(.85)}to{opacity:1;transform:scale(1)}}
