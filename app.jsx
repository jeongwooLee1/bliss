...
const sb = {
  async get(table, filter="") { 
    const hasSortCol = ["services","products","service_tags","service_categories","reservation_sources"].includes(table);
...
// ─── Constants ───
const BLISS_V = "2.62.0";
const uid = () => Math.random().toString(36).substr(2, 9);

// ─── 알림톡 발송 ───
const sendAlimtalk = async (templateCode, phone, params) => {
  try {
    const cfg = JSON.parse(localStorage.getItem("bliss_nhn_config")||"{}");
    if(!cfg.appKey||!cfg.secretKey||!cfg.senderKey||!templateCode||!phone) return;
    const proxyUrl = cfg.proxyUrl || `https://api-alimtalk.cloud.toast.com/alimtalk/v2.3/appkeys/${cfg.appKey}`;
    const r = await fetch(proxyUrl+"/messages", {
      method:"POST", headers:{"Content-Type":"application/json","X-Secret-Key":cfg.secretKey},
      body:JSON.stringify({ senderKey:cfg.senderKey, templateCode, recipientList:[{
        recipientNo:phone.replace(/-/g,""), templateParameter:params
      }]})
    });
    const res = await r.json();
    console.log("알림톡:", res.header?.isSuccessful?"성공":"실패", res);
  } catch(e) { console.warn("알림톡 발송 실패:", e.message); }
};
    ...
    try {
      ...
      const r = await fetch(proxyUrl+"/messages", {
        ...
        body:JSON.stringify({
          ...
          templateCode:cfg.tplReservation||"bliss_reservation",
          recipientList:[{recipientNo:cfg.testPhone.replace(/-/g,""),templateParameter:{
      ...
      else setTestResult({ok:false,msg:"❌ "+JSON.stringify(res.header?.resultMessage||res).slice(0,120)});
    } catch(e) {
      setTestResult({ok:false,msg:"❌ "+e.message+(e.message.includes("CORS")||e.message.includes("Failed to fetch")?" → 프록시 URL 설정이 필요합니다":"")});
    }
    setTesting(false);
  };
  ...
  return <div>
    ...
    <div className="card" style={{padding:20,maxWidth:540}}>
      ...
      <div style={{marginBottom:10}}>
        <div style={{fontSize:11,fontWeight:600,color:"#555",marginBottom:3}}>프록시 URL <span style={{color:"#ccc",fontWeight:400}}>(선택)</span></div>
        <input className="inp" value={cfg.proxyUrl||""} onChange={e=>up("proxyUrl",e.target.value)} placeholder="CORS 우회용. 미입력 시 NHN 직접 호출" style={{width:"100%",padding:"7px 10px",fontSize:12}}/>
        <div style={{fontSize:9,color:"#bbb",marginTop:2}}>Supabase Edge Function 또는 Cloudflare Worker URL</div>
      </div>
    ...
    <div className="card" style={{padding:20,maxWidth:540,marginTop:16}}>
      ...
      <div style={{fontSize:11,color:"#666",lineHeight:1.8}}>
        ...
        <strong>CORS 이슈 시:</strong> Supabase Edge Function 또는 Cloudflare Worker를 프록시로 사용하세요.
      </div>
    </div>
  </div>;
}
