              ...
              <div key={room.id} style={{width:colW,flexShrink:0,borderLeft:room.isNaver?"2px solid #A5D6A7":"1px solid #eee",background:room.isNaver?"#F1F8E9":(branchColor||"#fff")}}>
                ...
                <div style={{position:"relative",height:totalRows*rowH,cursor:room.isNaver?"default":(canEdit(room.branch_id)?"pointer":"default"),...gridBg}} onClick={e=>{const rect=e.currentTarget.getBoundingClientRect();handleCellClick(room,e.clientY-rect.top)}}>
                  ...
                  {roomBlocks.map(block => {
                    ...
                    const isDark = blockOp > 60;
                    const txtMain = isDark ? "#fff" : "#333";
                    const txtSub = isDark ? "rgba(255,255,255,.85)" : "#666";
                    const txtShadow = isDark ? "0 1px 2px rgba(0,0,0,.4)" : "none";
                    return (
                      <div key={block.id}
                        ...
                        {block.type==="reservation" && !block.isSchedule && <>
                          <div style={{display:"flex",alignItems:"center",gap:3,overflow:"hidden",whiteSpace:"nowrap"}}>
                            ...
                            <span style={{fontWeight:600,color:isNaverCancelled?"#999":txtMain,textShadow:txtShadow,overflow:"hidden",textOverflow:"ellipsis",textDecoration:isNaverCancelled?"line-through":"none"}}>{block.custGender && <span style={{color:isDark?(block.custGender==="M"?"#90caf9":"#ef9a9a"):(block.custGender==="M"?"#4a7cc8":"#e57373")}}>{block.custGender==="M"?"남":"여"}</span>} {block.custName}</span>
                          </div>
                          {block.selectedTags?.length>0 && <div style={{display:"flex",flexWrap:"wrap",gap:2,marginTop:1}}>
                            {block.selectedTags.slice(0,3).map(tid=>{const tg=tags.find(t=>t.id===tid);return tg?<span key={tid} style={{fontSize:Math.max(6,blockFs-2),padding:"0 3px",borderRadius:2,background:isDark?"rgba(255,255,255,.25)":(tg.color?tg.color+"25":"#7c7cc815"),color:isDark?"#fff":(tg.color||"#7c7cc8"),fontWeight:600,textShadow:txtShadow}}>{tg.name}</span>:null})}
                            {block.selectedTags.length>3 && <span style={{fontSize:Math.max(6,blockFs-2),color:txtSub}}>+{block.selectedTags.length-3}</span>}
                          </div>}
                          {block.selectedServices?.length>0 && <div style={{fontSize:Math.max(6,blockFs-2),color:isDark?"rgba(255,255,255,.9)":"#6bab9e",fontWeight:600,marginTop:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",textShadow:txtShadow}}>
                        ...
                        {block.type==="memo" && <div style={{color:isDark?"#fff":"#ef5350",fontWeight:600,textShadow:txtShadow}}><I name="fileText" size={10} color={isDark?"#fff":"#ef5350"}/> 메모</div>}
                        {block.type==="clockin" && <div style={{color:isDark?"#fff":"#d0d0d0",fontWeight:600,textShadow:txtShadow}}><I name="clock" size={10} color={isDark?"#fff":"#d0d0d0"}/> {staff?.dn||"출근"}</div>}
                        {block.type==="cleaning" && <div style={{color:isDark?"#fff":"#5cb5c5",fontWeight:600,textShadow:txtShadow}}><I name="sparkles" size={10} color={isDark?"#fff":"#5cb5c5"}/> 청소</div>}
                        {block.memo && (() => { const clean = block.memo.split("\n").filter(l => !/^\[등록:|^\[수정:/.test(l.trim())).join(" ").trim(); return clean ? <div style={{color:txtSub,marginTop:1,textShadow:txtShadow}}><I name="msgSq" size={10} color={isDark?"rgba(255,255,255,.7)":"#7c7cc8"}/> {clean}</div> : null; })()}
                        {/* Resize handle */}
                        ...
                        {isBeingResized && <div style={{position:"absolute",bottom:2,right:4,fontSize:Math.max(6,blockFs-2),fontWeight:700,color:isDark?"#fff":color,background:isDark?"rgba(0,0,0,.4)":"#fff",padding:"0 3px",borderRadius:2}}>
                          {blockDur}분
                        </div>}
                      </div>
                    );
  ...
  return (
    <div className="ov" onClick={onClose} style={{alignItems:"flex-start",paddingTop:20,overflow:"auto"}}>
      <div ref={modalRef} onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"8px",border:"1px solid #e0e0e0",
        width:"95%",maxWidth:960,maxHeight:"95vh",overflow:"hidden",animation:"fadeIn .2s ease",marginBottom:20,flexShrink:0,
        ...
        <div style={{padding:"16px 20px",borderTop:"1px solid #e0e0e0",flex:1,overflowY:"auto"}} className="form-col">
          ...
          {isSchedule && <>
            ...
            <FLD label={`기타일정 항목${tagDurTotal > 0 ? ` — 소요시간: ${tagDurTotal}분` : ""}`}>
              <div style={{display:"flex",flexWrap:"wrap",gap:5,padding:10,background:"#fff8f6",borderRadius:8,border:"1px dashed #e17055"}}>
                {visibleTags.length > 0 ? visibleTags.map(tag => {
                  ...
                  return <button key={tag.id} onClick={()=>toggleTag(tag.id)}
                    ...
                    {tag.name}
                    {tag.dur > 0 && <span style={{fontSize:9,opacity:0.7}}>({tag.dur}분)</span>}
                  </button>;
                }) : <span style={{fontSize:11,color:"#999",padding:8}}>서비스태그관리에서 기타일정(Y) 항목을 등록하세요</span>}
              </div>
              {(f.selectedTags||[]).length > 0 && <div style={{marginTop:6,display:"flex",flexWrap:"wrap",gap:4,alignItems:"center"}}>
                <span style={{fontSize:10,color:"#999"}}>선택:</span>
                {(f.selectedTags||[]).map(tid=>{const tag=tags.find(t=>t.id===tid);return tag?<span key={tid} style={{fontSize:10,padding:"2px 6px",borderRadius:3,background:tag.color?tag.color+"80":"#e1705530",color:tag.color?"#fff":"#e17055",fontWeight:600}}>{tag.name}{tag.dur>0?` ${tag.dur}분`:""}</span>:null})}
                {tagDurTotal>0 && <span style={{fontSize:10,color:"#e17055",fontWeight:700,marginLeft:4}}>= {tagDurTotal}분</span>}
              </div>}
            </FLD>
          </>}
          ...
          {!isSchedule && <>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 80px",gap:8}}>
              <FLD label="고객명"><input className="inp" value={f.custName} onChange={e=>set("custName",e.target.value)} placeholder="고객명"
                style={f.isNewCust?{borderColor:"#e57373",background:"#d0d0d008"}:{}}/></FLD>
              <FLD label="연락처"><input className="inp" value={f.custPhone} onChange={e=>set("custPhone",e.target.value)} placeholder="010"
                style={f.isNewCust?{borderColor:"#e57373",background:"#d0d0d008"}:{}}/></FLD>
              <FLD label="성별">
                <div style={{display:"flex",gap:2,height:34}}>
                  <button onClick={()=>set("custGender",f.custGender==="F"?"":"F")} style={{flex:1,fontSize:11,fontWeight:700,border:"1px solid #e57373",borderRadius:"4px 0 0 4px",cursor:"pointer",fontFamily:"inherit",
                    background:f.custGender==="F"?"#e57373":"transparent",color:f.custGender==="F"?"#fff":"#e57373"}}>여</button>
                  <button onClick={()=>set("custGender",f.custGender==="M"?"":"M")} style={{flex:1,fontSize:11,fontWeight:700,border:"1px solid #7c7cc8",borderRadius:"0 4px 4px 0",cursor:"pointer",fontFamily:"inherit",
            ...
            <FLD label={`상품/서비스${(tagDurTotal+svcDurTotal) > 0 ? ` — 합산 소요시간: ${tagDurTotal+svcDurTotal}분` : ""}`}>
              ...
              {(f.selectedTags||[]).length > 0 && <div style={{marginTop:6,display:"flex",flexWrap:"wrap",gap:4,alignItems:"center"}}>
                <span style={{fontSize:10,color:"#999"}}>선택:</span>
                {(f.selectedTags||[]).map(tid=>{const tag=tags.find(t=>t.id===tid);return tag?<span key={tid} style={{fontSize:10,padding:"2px 6px",borderRadius:3,background:tag.color?tag.color+"30":"#7c7cc820",color:tag.color||"#7c7cc8",fontWeight:600}}>{tag.name}{tag.dur>0?` ${tag.dur}분`:""}</span>:null})}
                {tagDurTotal>0 && <span style={{fontSize:10,color:"#ef5350",fontWeight:700,marginLeft:4}}>= {tagDurTotal}분</span>}
              </div>}
            </FLD>
            ...
            {(() => {
              ...
              return <FLD label={`시술 상품 선택${(f.selectedServices||[]).length > 0 ? ` (${(f.selectedServices||[]).length}건, ${svcDurTotal}분, ${fmt(svcPriceTotal)}원)` : ""}`}>
        ...
        <div style={{display:"flex",gap:6,padding:"12px 16px",borderTop:"1px solid #e0e0e0",background:"#fafafa",borderRadius:"0 0 8px 8px",flexShrink:0,flexWrap:"wrap"}}>
...
function DetailedSaleForm({ reservation, branchId, onSubmit, onClose, data, setData }) {
  ...
  // Auto-calc remaining for default payment (현금 = 총액 - 카드 - 입금 - 포인트)
  const svcRemain = Math.max(0, svcTotal - payMethod.svcCard - payMethod.svcTransfer - payMethod.svcPoint);
  const prodRemain = Math.max(0, prodTotal - payMethod.prodCard - payMethod.prodTransfer - payMethod.prodPoint);
  ...
  const handleSubmit = () => {
    ...
    const sale = {
      ...
      svcCash: svcRemain, svcTransfer: payMethod.svcTransfer, svcCard: payMethod.svcCard, svcPoint: payMethod.svcPoint,
      prodCash: prodRemain, prodTransfer: payMethod.prodTransfer, prodCard: payMethod.prodCard, prodPoint: payMethod.prodPoint,
...
function AdminServiceTags({ data, setData }) {
  ...
  const downloadXl = () => XL.download(tags.map((t,i)=>({태그명:t.name, 소요분:t.dur, 기타일정:t.scheduleYn, 색상:t.color||"", 사용:t.useYn!==false?"Y":"N"})), "서비스태그.xlsx", "태그", ["태그명","소요분","기타일정","색상","사용"]);
  const uploadXl = async (e) => {
    const file = e.target.files[0]; if(!file) return;
    const rows = await XL.readFile(file);
    const newTags = rows.map(r=>({id:uid(), name:r["태그명"]||r["name"]||"", dur:Number(r["소요분"]||r["dur"])||0, scheduleYn:r["기타일정"]||"N", color:r["색상"]||"", useYn:(r["사용"]||"Y")!=="N", sort:tags.length})).filter(r=>r.name);
  ...
  return <div>
    ...
    {showAdd && <div className="card" style={{padding:14,marginBottom:14}}>
      <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
        ...
        <FLD label="기타일정"><select className="inp" style={{width:55,...inpS}} value={newTag.scheduleYn} onChange={e=>setNewTag(p=>({...p,scheduleYn:e.target.value}))}><option value="N">N</option><option value="Y">Y</option></select></FLD>
