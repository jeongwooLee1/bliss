...
function App() {
  ...
  const [page, setPageRaw] = useState(() => {
    try { return sessionStorage.getItem("bliss_page") || "timeline"; } catch{ return "timeline"; }
  });
  const setPage = useCallback((p) => {
    setPageRaw(p);
    try { sessionStorage.setItem("bliss_page", p); } catch{}
  }, []);
  ...
  const handleLogout = () => {
    try{sessionStorage.removeItem("bliss_session");sessionStorage.removeItem("bliss_page");sessionStorage.removeItem("bliss_adminTab");}catch{}
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#e8e8f0 0%,#d8d8e8 50%,#c8c8d8 100%)",fontFamily:"'Pretendard',sans-serif",padding:20}}>
      ...
      <div style={{background:"#fff",borderRadius:12,border:"1px solid #e0e0e0",padding:40,width:"100%",maxWidth:420,animation:"fadeIn .5s ease",boxShadow:"0 8px 30px rgba(0,0,0,.1)"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{fontSize:9,color:"#d0d0d0",textAlign:"center",marginTop:8}}>v2.39.6</div>
        </div>
      </div>
    </div>
  );
...
function AdminPage({ data, setData, bizId }) {
  const [tab, setTabRaw] = useState(() => {
    try { return sessionStorage.getItem("bliss_adminTab") || "places"; } catch{ return "places"; }
  });
  const setTab = useCallback((t) => {
    setTabRaw(t);
    try { sessionStorage.setItem("bliss_adminTab", t); } catch{}
  }, []);
  return <div>
