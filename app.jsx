...
function TimelineModal({ item, onSave, onDelete, onDeleteRequest, onClose, selBranch, userBranches, data, setData, setPage }) {
  ...
  return (
    <div className="ov" onClick={onClose} style={{alignItems:"flex-start",paddingTop:window.innerWidth<=768?0:20,overflow:"auto",WebkitOverflowScrolling:"touch",alignItems:window.innerWidth<=768?"flex-end":"flex-start"}}>
      <div ref={modalRef} className="modal-res" onClick={e=>e.stopPropagation()} style={{background:"#fff",
        borderRadius:window.innerWidth<=768?"16px 16px 0 0":"8px",border:"1px solid #e0e0e0",
        width:window.innerWidth<=768?"100%":"95%",maxWidth:window.innerWidth<=768?"100%":500,
        maxHeight:window.innerWidth<=768?"95vh":"95vh",overflow:"hidden",
        animation:window.innerWidth<=768?"slideUp .3s ease":"fadeIn .2s ease",
        marginBottom:window.innerWidth<=768?0:20,flexShrink:0,
        boxShadow:"0 -4px 30px rgba(0,0,0,.15)",display:"flex",flexDirection:"column"}}>
        {/* ═══ Chrome-style Tabs ═══ */}
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:768px){
    ...
    .ov{padding:0!important;-webkit-overflow-scrolling:touch!important}
    .modal-res{border-radius:16px 16px 0 0!important;max-width:100%!important;width:100%!important}
    ...
    .timeline-scroll{scroll-snap-type:x mandatory;-webkit-scroll-snap-type:x mandatory}
    .tl-room-col{scroll-snap-align:start}
