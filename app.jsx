...
function QuickBookModal({ onClose, onParsed, data }) {
  ...
  const [audioData, setAudioData] = useState(null); // {base64, mimeType}
  const fileRef = useRef(null);

  const apiKey = localStorage.getItem("bliss_gemini_key") || "";

  // Audio recording → send directly to Gemini
    ...
    try {
      ...
      rec.onstop = () => {
        ...
        reader.onload = () => {
          ...
          setAudioData({base64, mimeType: mime.split(";")[0]});
        };
        reader.readAsDataURL(blob);
      };
      rec.start(1000);
      mediaRecRef.current = rec;
      setIsListening(true); setRecordSec(0); setAudioData(null);
...
[음성 분석 시 주의사항]
- 오디오가 첨부되면 음성을 텍스트로 변환하고 예약 정보를 추출하세요
- 한국어 이름, 전화번호(공일공 = 010), 날짜, 시간을 정확히 인식하세요
- "공"=0, "일"=1, "이"=2, "삼"=3, "사"=4, "오"=5, "육"=6, "칠"=7, "팔"=8, "구"=9
- 모호한 발음은 문맥에 맞게 해석하세요
...
한글 숫자(공,일,이 등)는 아라비아 숫자로 변환하세요.`;
  ...
  const doParse = async () => {
    ...
    if (!input.trim() && !imgData && !audioData) { setError("텍스트, 음성, 또는 이미지를 입력하세요"); return; }
  ...
  return <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div style={{background:"#fff",borderRadius:16,width:"100%",maxWidth:440,maxHeight:"90vh",overflow:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.2)"}}>
      ...
      <div style={{padding:20}}>
        {/* Text input */}
        {mode==="text" && <div style={{marginBottom:16}}>
        ...
        {/* Voice recording */}
        {mode==="voice" && <div style={{marginBottom:16}}>
          <div style={{textAlign:"center",padding:20,background:"#fafafa",borderRadius:12,border:"1px solid #e0e0e0"}}>
            {isListening ? <>
              <div style={{fontSize:48,marginBottom:8,animation:"pendingBlink 1s infinite"}}>🔴</div>
              <div style={{fontSize:24,fontWeight:800,color:"#ef4444",fontFamily:"monospace",marginBottom:12}}>
                {Math.floor(recordSec/60)}:{String(recordSec%60).padStart(2,"0")}
              </div>
              <div style={{fontSize:12,color:"#888",marginBottom:16}}>말씀해주세요... (이름, 전화번호, 날짜, 시간)</div>
              <button onClick={stopVoice} style={{...btnBase,justifyContent:"center",width:"100%",background:"#ef4444",color:"#fff",padding:"12px 20px",fontSize:14,borderRadius:10}}>
                ⏹️ 녹음 중지
              </button>
            </> : audioData ? <>
              <div style={{fontSize:48,marginBottom:8}}>✅</div>
              <div style={{fontSize:14,fontWeight:700,color:"#22c55e",marginBottom:4}}>녹음 완료 ({recordSec}초)</div>
              <div style={{fontSize:11,color:"#888",marginBottom:16}}>아래 "AI 분석" 버튼을 눌러 음성을 분석하세요</div>
              <button onClick={()=>{setAudioData(null);setRecordSec(0);}} style={{fontSize:11,color:"#888",background:"none",border:"1px solid #ddd",borderRadius:6,padding:"4px 12px",cursor:"pointer",fontFamily:"inherit"}}>
                다시 녹음
              </button>
            </> : <>
              <div style={{fontSize:48,marginBottom:12}}>🎤</div>
              <div style={{fontSize:12,color:"#888",marginBottom:16}}>버튼을 누르고 예약 정보를 말씀하세요</div>
              <button onClick={startVoice} style={{...btnBase,justifyContent:"center",width:"100%",background:"#f0f0ff",color:"#7c3aed",border:"1px solid #c4b5fd",padding:"12px 20px",fontSize:14,borderRadius:10}}>
                🎤 녹음 시작
              </button>
              <div style={{fontSize:10,color:"#aaa",marginTop:10}}>Gemini AI가 음성을 직접 인식합니다 (한국어/영어 지원)</div>
            </>}
          </div>
        </div>}
        ...
        {result && <div style={{marginTop:16}}>
          ...
          <div style={{display:"flex",gap:8,marginTop:14}}>
            <button onClick={()=>{setResult(null);setError(null);setAudioData(null);setRecordSec(0);}} style={{...btnBase,flex:1,justifyContent:"center",background:"#f5f5f5",color:"#666",border:"1px solid #ddd"}}>다시 분석</button>
