...
function App() {
  ...
  useEffect(() => {
    ...
    // 폴링 제어
    const stopPoll = () => { clearInterval(pollIv); pollIv = null; };
    const startPoll = (interval) => { clearInterval(pollIv); pollIv = setInterval(poll, interval); };

    // Realtime 시도
    if (supaClient) {
      // 기존 채널 정리
      try { supaClient.getChannels().forEach(ch => supaClient.removeChannel(ch)); } catch(e){}
      try {
        channel = supaClient.channel("bliss-res-" + currentBizId + "-" + Date.now())
          ...
          .subscribe((status, err) => {
            ...
            if (status === "SUBSCRIBED") {
              stopPoll();
              console.log("✅ Realtime 연결 성공 (폴링 OFF)");
            }
            if (status === "CLOSED" || status === "TIMED_OUT" || status === "CHANNEL_ERROR") {
              startPoll(20000);
              console.log("⚠️ Realtime 실패, 폴링 20초 ON");
            }
          });
      } catch(e) { console.warn("Realtime init error:", e); startPoll(20000); }
    } else {
      startPoll(20000); // supabase 라이브러리 없으면 폴링만
    }
