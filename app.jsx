...
function SuperDashboard({ superData, setSuperData, currentUser, onLogout, onEnterBiz }) {
  ...
  const SideContent = () => <>
    ...
    <div style={{flex:1,padding:"12px 0"}}>
      {tabs.map(t=>(
        <button key={t.id} onClick={()=>{setTab(t.id);setSideOpen(false)}} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 16px",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",fontSize:13,fontWeight:tab===t.id?700:400,
    ...
    <div style={{display:"flex",height:"100vh",fontFamily:"'Pretendard',sans-serif",background:"#f8f8f8"}}>
      ...
      <main className="main-c" style={{flex:1,display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden"}}>
        ...
        <div className="mob-hdr" style={{padding:"10px 16px",background:"#fff",borderBottom:"1px solid #e0e0e0",display:"flex",alignItems:"center",gap:12}}>
          <button onClick={()=>setSideOpen(true)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",color:"#333",cursor:"pointer",fontSize:20,fontFamily:"inherit"}}><I name="menu" size={20}/></button>
    ...
    <div style={{flex:1,padding:"8px 0",overflowY:"auto"}}>
      {cats.map((cat,ci) => (
        <div key={ci}>
          ...
          {cat.items.map(n=>(
            <button key={n.id} onClick={()=>setPage(n.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 16px",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",fontSize:13,fontWeight:page===n.id?700:400,
...
function Timeline({ data, setData, userBranches, viewBranches=[], isMaster, currentUser, setPage, bizId }) {
  ...
  // Time config - 5min intervals (persist to localStorage)
  const tlKey = "bliss_tl_";
  const tlLoad = (k, def) => { try { const v = localStorage.getItem(tlKey+k); return v !== null ? Number(v) : def; } catch(e) { return def; } };
  const tlSave = (k, v) => { try { localStorage.setItem(tlKey+k, v); } catch(e) {} };
  const [startHour, setStartHourRaw] = useState(() => tlLoad("sh", 8));
  const [endHour, setEndHourRaw] = useState(() => tlLoad("eh", 22));
  const [rowH, setRowHRaw] = useState(() => tlLoad("rh", 14));
  const [colW, setColWRaw] = useState(() => tlLoad("cw", 160));
  const [timeUnit, setTimeUnitRaw] = useState(() => tlLoad("tu", 5));
  const [blockFs, setBlockFsRaw] = useState(() => tlLoad("fs", 10));
  const [blockOp, setBlockOpRaw] = useState(() => tlLoad("op", 100));
  const [statusClr, setStatusClrRaw] = useState(() => getStatusClr());
  const setStatusClr = (k,v) => { setStatusClrRaw(p => { const n = {...p,[k]:v}; try{localStorage.setItem("tl_sc",JSON.stringify(n))}catch(e){} return n; }); };
  const setStartHour = v => { const nv = typeof v === "function" ? v(startHour) : v; setStartHourRaw(nv); tlSave("sh", nv); };
  const setEndHour = v => { const nv = typeof v === "function" ? v(endHour) : v; setEndHourRaw(nv); tlSave("eh", nv); };
  const setRowH = v => { const nv = typeof v === "function" ? v(rowH) : v; setRowHRaw(nv); tlSave("rh", nv); };
  const setColW = v => { const nv = typeof v === "function" ? v(colW) : v; setColWRaw(nv); tlSave("cw", nv); };
  const setTimeUnit = v => { const nv = typeof v === "function" ? v(timeUnit) : v; setTimeUnitRaw(nv); tlSave("tu", nv); };
  const setBlockFs = v => { const nv = typeof v === "function" ? v(blockFs) : v; setBlockFsRaw(nv); tlSave("fs", nv); };
  const setBlockOp = v => { const nv = typeof v === "function" ? v(blockOp) : v; setBlockOpRaw(nv); tlSave("op", nv); };
    ...
    <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
      ...
      <div className="tl-topbar" style={{borderBottom:"1px solid #e0e0e0",background:"#fff",flexShrink:0,padding:"6px 12px",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
          <button onClick={()=>changeDate(-1)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",fontSize:12,color:"#666",padding:"2px 4px",flexShrink:0}}><I name="chevL" size={14}/></button>
          ...
          <button onClick={()=>changeDate(1)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",fontSize:12,color:"#666",padding:"2px 4px",flexShrink:0}}><I name="chevR" size={14}/></button>
      ...
      <div ref={scrollRef} className="timeline-scroll" style={{flex:1,overflow:"scroll",position:"relative",minHeight:0}}>
        <div style={{display:"flex",minWidth:"fit-content"}}>
          ...
          {allRooms.map((room, ci) => {
            ...
            return (
              <div key={room.id} style={{width:colW,flexShrink:0,borderLeft:room.isNaver?"2px solid #A5D6A7":"1px solid #eee",background:room.isNaver?"#F1F8E9":(branchColor||"#fff")}}>
                {/* Header */}
          ...
          <div style={{padding:"8px 0"}}>
            {[
              ...
              <button key={opt.mode} onClick={()=>handleDeleteConfirm(opt.mode)}
                style={{width:"100%",padding:"12px 24px",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",background:"transparent",cursor:"pointer",
        ...
        <div style={{display:"flex",alignItems:"flex-end",gap:0,padding:"0",background:"#f5f5f5",borderRadius:"8px 8px 0 0",position:"sticky",top:0,zIndex:10,borderBottom:"1px solid #e0e0e0"}}>
          ...
          <button onClick={()=>{setIsSchedule(false);setF(p=>({...p,isSchedule:false,selectedTags:[],type:"reservation"}));if(modalRef.current)modalRef.current.scrollTop=0}}
            style={{flex:1,padding:"12px 16px",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",
              border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",borderBottom:isSchedule?"none":"2px solid #7c7cc8",
          ...
          <button onClick={()=>{setIsSchedule(true);setF(p=>({...p,isSchedule:true,selectedTags:[],type:"reservation"}));if(modalRef.current)modalRef.current.scrollTop=0}}
            style={{flex:1,padding:"12px 16px",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",
              border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",borderBottom:isSchedule?"2px solid #e17055":"none",
              background:isSchedule?"#fff":"transparent",color:isSchedule?"#e17055":"#999",
              transition:"all .15s"}}>
            <I name="clipboard" size={12}/> 내부일정
          </button>
          {/* 닫기 */}
          <button onClick={onClose} style={{padding:"8px 14px",background:"transparent",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",color:"#999",cursor:"pointer",
          ...
          {f.status === "pending" && <div style={{background:"#FFF3E0",border:"1.5px solid #FF9800",borderRadius:6,padding:"8px 12px",marginBottom:12,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",animation:"naverBlink 1.5s infinite"}}>
            ...
            <div style={{marginLeft:"auto",display:"flex",gap:6}}>
              <button onClick={()=>{if(!window.confirm("이 예약을 취소 처리하시겠습니까?")) return; const updated={...f,status:"naver_cancelled"}; onSave(updated); onClose();}}
                style={{fontSize:11,color:"#fff",fontWeight:700,background:"#E65100",padding:"5px 12px",borderRadius:6,border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",fontFamily:"inherit"}}>취소처리</button>
              {(() => {
        ...
        <div style={{padding:"12px 16px",borderTop:"1px solid #e0e0e0",background:"#fafafa",borderRadius:"0 0 8px 8px",flexShrink:0}}>
            ...
            {!isSchedule && f.type === "reservation" && (
              <button onClick={()=>setShowSaleForm(true)}
                style={{padding:"10px 18px",borderRadius:8,border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",background:existingSale?"linear-gradient(135deg,#5cb5c5,#6bab9e)":"linear-gradient(135deg,#7c7cc8,#5cb5c5)",color:"#fff",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap"}}>
          ...
          res.map(r=>{
            ...
            return <tr key={r.id}>
              ...
              <td><div style={{display:"flex",gap:3}}>
                {r.status==="confirmed"&&<button className="btn-sm" style={{background:"#e8f5f0",color:"#6bab9e",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:11}} onClick={()=>updateStatus(r.id,"completed")}><I name="check" size={11}/> 완료</button>}
...
function CustomersPage({ data, setData, userBranches, isMaster }) {
  ...
  return <div>
    ...
    {showModal && <CustModal item={editItem} onSave={handleSave} onClose={()=>{setShowModal(false);setEditItem(null)}} defBranch={userBranches[0]} userBranches={userBranches} branches={data.branches||[]}/>}
  </div>;
}
        ...
        <tbody onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
          {branches.map((b,i) => (
            <tr key={b.id}
              ...
              <td>
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  <input type="color" value={b.color||"#FFFFFF"} onChange={e=>updateField(b.id,"color",e.target.value)} style={{width:24,height:24,border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",background:"none",cursor:"pointer",padding:0}}/>
                ...
                {deleteId===b.id ? (
                  <div style={{display:"flex",gap:3,alignItems:"center"}}>
                    <button onClick={()=>deleteBranch(b.id)}
                      style={{padding:"2px 8px",fontSize:10,fontWeight:700,borderRadius:4,border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",background:"#e57373",color:"#fff",cursor:"pointer",fontFamily:"inherit"}}>확인</button>
                  ...
                  <button onClick={()=>setDeleteId(b.id)}
                    style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",fontSize:13,color:"#e57373"}}><I name="trash" size={13}/></button>
                )}
              </td>
            </tr>
          ))}
      ...
      <table><thead><tr>
        ...
        {rooms.map((r,i) => {
          if(editId===r.id) return <tr key={r.id} style={{background:"#f5f5ff"}}>
            ...
            <td><input type="color" value={editData.color||"#FFFFFF"} onChange={e=>setEditData(p=>({...p,color:e.target.value}))} style={{width:28,height:24,border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer"}}/></td>
          ...
          return <tr key={r.id}>
            ...
            <td><div style={{display:"flex",gap:4}}>
              <button onClick={()=>startEdit(r)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",color:"#7c7cc8",fontSize:13}}><I name="edit" size={13}/></button>
              {delId===r.id
                ...
                : <button onClick={()=>setDelId(r.id)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",color:"#e57373",fontSize:13}}><I name="trash" size={13}/></button>}
            </div></td>
          </tr>;
        })}
      </tbody></table>
        ...
        {filtered.map((svc,i)=>{
          ...
          return <tr key={svc.id} draggable onDragStart={()=>drag.onDragStart(i)} onDragEnter={()=>drag.onDragEnter(i)} onDragEnd={drag.onDragEnd} onDragOver={e=>e.preventDefault()}>
            ...
            <td style={{textAlign:"center"}}><div style={{display:"flex",gap:4,justifyContent:"center"}}>
              <button onClick={()=>startEdit(svc)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",color:"#7c7cc8",fontSize:13}}><I name="edit" size={13}/></button>
              {delConfirm===svc.id
                ...
                : <button onClick={()=>setDelConfirm(svc.id)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",color:"#e57373",fontSize:13}}><I name="trash" size={13}/></button>}
            </div></td>
          </tr>;
        })}
      </tbody></table>
      ...
      <table><thead><tr>
        ...
        <tbody>{items.map((it,i)=>{
          ...
          return <tr key={it.id} draggable onDragStart={()=>drag.onDragStart(i)} onDragEnter={()=>drag.onDragEnter(i)} onDragEnd={drag.onDragEnd} onDragOver={e=>e.preventDefault()}>
            ...
            <td style={{textAlign:"center"}}><div style={{display:"flex",gap:4,justifyContent:"center"}}>
              <button onClick={()=>startEdit(it)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",color:"#7c7cc8",fontSize:13}}><I name="edit" size={13}/></button>
              {delConfirm===it.id
                ...
                : <button onClick={()=>setDelConfirm(it.id)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",color:"#e57373",fontSize:13}}><I name="trash" size={13}/></button>}
            </div></td>
          </tr>;
        })}</tbody>
      </table>
    ...
    {showAdd && <div className="card" style={{padding:14,marginBottom:14}}>
      <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
        ...
        <FLD label="색상"><div style={{display:"flex",alignItems:"center",gap:3}}>
          <input type="color" value={newTag.color||"#e0e0e0"} onChange={e=>setNewTag(p=>({...p,color:e.target.value}))} style={{width:22,height:22,border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer"}}/>
      ...
      {tags.map((tag,i) => {
        ...
        if (isEdit) return <tr key={tag.id} style={{background:"#f5f5ff"}}>
          ...
          <td><div style={{display:"flex",alignItems:"center",gap:2}}>
            <input type="color" value={editData.color||"#e0e0e0"} onChange={e=>setEditData(p=>({...p,color:e.target.value}))} style={{width:20,height:18,border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer"}}/>
        ...
        return <tr key={tag.id} draggable onDragStart={()=>drag.onDragStart(i)} onDragEnter={()=>drag.onDragEnter(i)} onDragEnd={drag.onDragEnd} onDragOver={e=>e.preventDefault()}
          ...
          <td style={{textAlign:"center"}}><div style={{display:"flex",gap:3,justifyContent:"center"}}>
            <button onClick={()=>startEdit(tag)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",color:"#7c7cc8",fontSize:12}}><I name="edit" size={13}/></button>
            {delConfirm===tag.id
              ...
              : <button onClick={()=>setDelConfirm(tag.id)} style={{background:"none",border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0",cursor:"pointer",color:"#e57373",fontSize:12}}><I name="trash" size={13}/></button>}
          </div></td>
        </tr>;
      })}</tbody></table>
    </div>
...
const S = {
  ...
  menuBtn: { background:"none", border:sel?`1px solid ${bgClr}`:"1px solid #d0d0d0", color:"#333", cursor:"pointer", fontSize:20, fontFamily:"inherit" },
};
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:768px){
    ...
    .modal-res{max-height:none!important;overflow:visible!important}
