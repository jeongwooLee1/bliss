...
const DB_COLS={
  reservations:["id","business_id","bid","room_id","cust_id","cust_name","cust_phone","cust_gender","staff_id","service_id","date","time","dur","status","memo","type","is_schedule","is_new_cust","selected_tags","selected_services","repeat","repeat_until","repeat_group_id","reservation_id","updated_at","source"],
...
async function loadAllFromDb(bizId) {
  ...
  let custPackages = [];
  try { custPackages = await sb.getByBiz("customer_packages",bizId); } catch(e) { console.warn("customer_packages load failed:",e); }
  return { branches:fromDb("branches",branches), rooms, users:fromDb("app_users",users), customers:fromDb("customers",customers),
    ...
    products, cats, resSources:fromDb("reservation_sources",resSources||[]), custPackages: custPackages||[] };
...
const sendAlimtalk = async (templateCode, phone, params, action, branchConfig) => {
  try {
    ...
    if(phone) payload.recipientList = [{recipientNo:phone.replace(/-/g,""), templateParameter:params||{}}];
    const r = await fetch(SB_URL+"/functions/v1/send-alimtalk", {
      method:"POST", headers:{"Content-Type":"application/json","Authorization":"Bearer "+SB_KEY},
      body:JSON.stringify(payload)
    });
    const res = await r.json();
    console.log("알림톡:", res.header?.isSuccessful?"성공":"실패", res);
    return res;
  } catch(e) { console.warn("알림톡 발송 실패:", e.message); return null; }
};
  ...
  return (
    <div style={S.root}>
      ...
      <main className="main-c" style={S.main}>
        <div className="mob-hdr" style={{...S.mobHdr, ...(page==="timeline"?{display:"none"}:{})}}>
        ...
        <div className="page-pad" style={{flex:1,padding:page==="timeline"?"0":"16px 20px 16px",display:"flex",flexDirection:"column",minHeight:0,overflow:"hidden"}}>
          <div className={page==="timeline"?"":"fade-in"} key={page} style={page==="timeline"?{flex:1,display:"flex",flexDirection:"column",minHeight:0}:{overflow:"auto",flex:1,WebkitOverflowScrolling:"touch"}}>
            {page==="timeline" && <Timeline data={data} setData={setData} userBranches={userBranches} viewBranches={viewBranches} isMaster={isMaster} currentUser={currentUser} setPage={setPage} bizId={currentBizId} onMenuClick={()=>setSideOpen(true)} bizName={bizName}/>}
...
function Timeline({ data, setData, userBranches, viewBranches=[], isMaster, currentUser, setPage, bizId, onMenuClick, bizName }) {
  ...
  const naverAssignments = (() => {
    ...
    branchesToShow.forEach(br => {
      ...
      const regularRooms = allRooms.filter(r => !r.isNaver && r.branch_id === br.id);
      ...
      if (naverRooms.length === 0) {
        brBlocks.filter(b => !isNaverRes(b) && !b.isSchedule).forEach(b => {
          if (roomOcc.has(b.roomId)) roomOcc.get(b.roomId).push({ s: toMin(b.time), e: toMin(b.time) + (b.dur||30) });
        });
      }
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",flex:1,minHeight:0}}>
      ...
      <div ref={scrollRef} className="timeline-scroll" style={{flex:1,overflow:"auto",minHeight:0,overscrollBehavior:"none"}}>
        ...
        {window.innerWidth<=768 && onMenuClick && <div style={{padding:"2px 16px",display:"flex",alignItems:"center",gap:12,background:"#fff",position:"sticky",left:0,width:"100vw",boxSizing:"border-box"}}>
        ...
        {/* Top Bar - sticky */}
        <div ref={topbarRef} className="tl-topbar" style={{position:"sticky",top:0,left:0,zIndex:30,borderBottom:"1px solid #e0e0e0",background:"#fff",padding:"6px 12px",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap",width:"100vw",boxSizing:"border-box",overflow:"visible"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0,flexWrap:"wrap",maxWidth:"100%"}}>
          ...
          <button onClick={()=>setShowQuickBook(true)} style={{padding:"0 12px",height:32,fontSize:12,border:"none",borderRadius:16,background:"linear-gradient(135deg,#4285f4,#9b72cb,#d96570)",color:"#fff",cursor:"pointer",fontFamily:"inherit",flexShrink:0,display:"flex",alignItems:"center",gap:5,fontWeight:700,boxShadow:"0 2px 8px rgba(66,133,244,.25)"}}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="#fff" style={{flexShrink:0}}><path d="M12 2L13.09 8.26L18 6L14.74 10.91L21 12L14.74 13.09L18 18L13.09 15.74L12 22L10.91 15.74L6 18L9.26 13.09L3 12L9.26 10.91L6 6L10.91 8.26L12 2Z"/></svg> AI Book
          </button>
      ...
      {(() => {
        ...
        <div style={{display:"flex",minWidth:"fit-content",position:"relative"}}>
          ...
          <div className="tl-time-col" style={{width:timeLabelsW,flexShrink:0,position:"sticky",left:0,zIndex:20,background:"#fff",borderRight:"1px solid #eee"}}>
            <div style={{height:headerH,borderBottom:"1px solid #eee",position:"sticky",top:topbarH,zIndex:25,background:"#fff"}}/>
          ...
          {allRooms.map((room, ci) => {
            const roomBlocks = blocks.filter(b => {
              ...
              // 일반 칼럼: 네이버칼럼이 없는 지점이면 네이버 예약도 roomId 기준으로 표시
              const hasNaverCols = allRooms.some(r => r.isNaver && r.branch_id === room.branch_id);
              if (isNaverRes(b) && hasNaverCols) return false;
            ...
            return (
              <div key={room.id} className="tl-room-col" style={{width:colW,flexShrink:0,borderLeft:room.isNaver?"2px solid #A5D6A7":(isNewBranch&&ci>0?"none":"1px solid #f0f0f0"),background:room.isNaver?"#F1F8E9":(branchColor||"#fff"),marginLeft:isNewBranch&&ci>0?4:0,boxShadow:isNewBranch&&ci>0?"-4px 0 8px rgba(0,0,0,.06)":room.isNaver?"inset 2px 0 4px rgba(76,175,80,.08)":"none"}}>
                {/* Room Header - sticky */}
                <div style={{height:headerH,borderBottom:"1px solid #eee",position:"sticky",top:topbarH,zIndex:10,background:room.isNaver?"#E8F5E9":(branchColor||"#fff"),display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",lineHeight:1.2}}>
...
function TimelineModal({ item, onSave, onDelete, onDeleteRequest, onClose, selBranch, userBranches, data, setData, setPage }) {
  ...
  const [f, setF] = useState(isNew && !item?.id ? {
    ...
    return {...item, memo: cleanMemo, custGender: item?.custGender || "", endDate: item?.date||todayStr(), endTime: defaultEnd(),
      ...
      source: item?.source || (isNaverItem ? "네이버" : "")};
  })());
            ...
            {(() => {
              ...
              <button onClick={()=>setShowSvcPicker(!showSvcPicker)}
                ...
                <span>{(f.selectedServices||[]).length > 0 ?
                  `${(f.selectedServices||[]).map(sid=>SVC_LIST.find(s=>s.id===sid)?.name).filter(Boolean).join(", ")} (${(f.selectedServices||[]).length}건, ${svcDurTotal}분${!hasGender && hasGenderDep ? "" : ", "+fmt(svcPriceTotal)+"원"})`
                  : "시술 상품을 선택하세요"}</span>
              ...
              {showSvcPicker && <div style={{marginTop:6,border:"1px solid #e0e0e0",borderRadius:8,background:"#fff",maxHeight:280,overflow:"auto"}}>
                ...
                {SVC_LIST.length>0 && SVC_LIST.map(svc=>{
                      ...
                      return <div key={svc.id} onClick={()=>!disabled&&toggleService(svc.id)}
                        ...
                        <span style={{fontSize:10,color:price===null?"#ccc":"#ef5350",fontWeight:600,width:55,textAlign:"right"}}>{price===null?"성별?":price>0?fmt(price):"-"}</span>
                      </div>;
                    })}
              </div>}
            </div>;
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div className="sale-grid" style={{ flex: 1, overflow: "auto", padding: "10px 14px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 14, alignContent: "start" }}>
          ...
          <div>
            ...
            {leftSvcs.map(svc => { const it=items[svc.id]||{}; const dp=gender?(gender==="M"?svc.priceM:svc.priceF):(svc.priceF===svc.priceM?svc.priceF:0); return <SaleSvcRow key={svc.id} id={svc.id} name={svc.name} dur={svc.dur} checked={!!it.checked} amount={it.amount||0} defPrice={dp} toggle={toggle} setAmt={setAmt} />; })}
          ...
          <div>
            {rightSvcs.map(svc => { const it=items[svc.id]||{}; const dp=gender?(gender==="M"?svc.priceM:svc.priceF):(svc.priceF===svc.priceM?svc.priceF:0); return <SaleSvcRow key={svc.id} id={svc.id} name={svc.name} dur={svc.dur} checked={!!it.checked} amount={it.amount||0} defPrice={dp} toggle={toggle} setAmt={setAmt} />; })}
            {/* Discount */}
            ...
            return <React.Fragment key={c.id}>
              ...
              {isDetail && <tr><td colSpan={10} style={{padding:0,background:"#f8f8ff"}}>
                <div style={{padding:"12px 16px"}}>
                  {/* 다회권 보유 현황 */}
                  {(() => {
                    const pkgs = (data.custPackages||[]).filter(p=>p.customer_id===c.id && (p.total_count-p.used_count)>0);
                    const pkgSvcs = (data.services||[]).filter(s=>s.isPackage);
                    return <div style={{marginBottom:12}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                        <div style={{fontSize:12,fontWeight:700,color:"#9b72cb"}}><I name="tag" size={12}/> 다회권 ({pkgs.length}건)</div>
                        {pkgSvcs.length>0 && <select style={{fontSize:10,padding:"3px 6px",borderRadius:4,border:"1px solid #d0d0d0",color:"#7c7cc8"}}
                          value="" onChange={e=>{
                            if(!e.target.value) return;
                            const svc = pkgSvcs.find(s=>s.id===e.target.value);
                            if(!svc) return;
                            const pkg = {id:uid(), business_id:_activeBizId, customer_id:c.id, service_id:svc.id, service_name:svc.name, total_count:svc.pkgCount||5, used_count:0, purchased_at:new Date().toISOString(), note:""};
                      ...
                      {pkgs.length===0 ? <div style={{fontSize:11,color:"#999",padding:4}}>보유 다회권 없음</div> :
                        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                          {pkgs.map(p=>{
                            const remain = p.total_count - p.used_count;
                            const pct = (remain/p.total_count)*100;
                            return <div key={p.id} style={{border:"1px solid #e0e0e0",borderRadius:8,padding:"8px 12px",background:"#fff",minWidth:140}}>
                              <div style={{fontSize:11,fontWeight:700,color:"#333"}}>{p.service_name}</div>
                              <div style={{display:"flex",alignItems:"center",gap:6,marginTop:4}}>
                                <div style={{flex:1,height:6,background:"#eee",borderRadius:3,overflow:"hidden"}}>
                                  <div style={{width:pct+"%",height:"100%",background:pct>30?"linear-gradient(90deg,#4285f4,#9b72cb)":"#e57373",borderRadius:3,transition:"width .3s"}}/>
                                </div>
                                <span style={{fontSize:12,fontWeight:800,color:pct>30?"#7c7cc8":"#e57373"}}>{remain}<span style={{fontSize:9,color:"#999"}}>/{p.total_count}</span></span>
                              </div>
                              <div style={{display:"flex",gap:4,marginTop:6}}>
                                ...
                                }} style={{fontSize:9,padding:"2px 8px",borderRadius:4,border:"1px solid #7c7cc8",background:"#7c7cc810",color:"#7c7cc8",cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>1회 사용</button>
                                <button onClick={()=>{
                                ...
                                }} style={{fontSize:9,padding:"2px 6px",borderRadius:4,border:"1px solid #e57373",background:"#fff",color:"#e57373",cursor:"pointer",fontFamily:"inherit"}}>삭제</button>
                              </div>
                            </div>;
                          })}
                        </div>}
...
function AdminPage({ data, setData, bizId }) {
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:2,marginBottom:16,background:"#e0e0e0",borderRadius:8,padding:3,width:"fit-content",flexWrap:"wrap"}}>
      {[["places","예약장소관리"],["workers","담당자관리"],["saleitems","시술상품관리"],["prodmgmt","제품관리"],["svctags","서비스태그관리"],["ressrc","예약경로관리"],["notiSettings","알림톡설정"],["aisettings","AI설정"]].map(([k,v])=>(
        <button key={k} className={tab===k?"btn-p btn-sm":"btn-s btn-sm"} onClick={()=>setTab(k)}>{v}</button>
      ))}
    </div>
    ...
    {tab==="notiSettings" && <AdminNotiSettings data={data} setData={setData}/>}
...
function AdminPlaces({ data, setData, bizId }) {
  ...
  const applyRow = (id) => {
    ...
    if(b) sb.update("branches", id, {name:b.name, short:b.short||b.name, phone:b.phone||"", address:b.address||"", color:b.color||"#FFFFFF", use_yn:b.useYn!==false, naver_email:b.naverEmail||"", naver_biz_id:b.naverBizId||"", naver_col_count:b.naverColCount||0, noti_config:b.notiConfig?JSON.stringify(b.notiConfig):"{}"}).catch(console.error);
  ...
  // Excel download
  const downloadExcel = () => {
    const rows = branches.map((b,i) => ({지점명:b.name, 약칭:b.short||"", 전화:b.phone||"", 주소:b.address||"", 색상:b.color||"", 사용:b.useYn!==false?"Y":"N", 네이버Gmail:b.naverEmail||"", 네이버BizID:b.naverBizId||"", 네이버칼럼:b.naverColCount||""}));
    XL.download(rows, "지점목록.xlsx", "지점", ["지점명","약칭","전화","주소","색상","사용","네이버Gmail","네이버BizID","네이버칼럼"]);
  };
  // Excel upload
  const uploadExcel = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      const newBranches = rows.map((r,i) => ({
        ...
        naverEmail:r["네이버Gmail"]||"", naverBizId:r["네이버BizID"]||"", naverColCount:Number(r["네이버칼럼"])||0
      }));
      if(newBranches.length && confirm(`${newBranches.length}개 지점을 추가하시겠습니까?`)) {
        newBranches.forEach(b => sb.insert("branches", {id:b.id, business_id:bizId||_activeBizId, name:b.name, short:b.short, phone:b.phone, address:b.address, color:b.color, sort:0, use_yn:b.useYn, naver_email:b.naverEmail, naver_biz_id:b.naverBizId, naver_col_count:b.naverColCount}).catch(console.error));
  ...
  const handleDragEnd = () => {
    if (dragIdx !== null && dragOverIdx !== null && dragIdx !== dragOverIdx) {
      ...
      setData(prev => ({...prev, branchSettings: reordered, branches: reordered}));
      reordered.forEach((b,i) => sb.update("branches", b.id, {sort: i}).catch(console.error));
    }
    ...
    <div className={isMobileView?undefined:"card"} style={{padding:isMobileView?0:20}}>
      ...
      {!isMobileView && <div className="tw">
        <table><thead><tr>
          ...
          <th style={{minWidth:130}}>지점명</th>
          <th style={{minWidth:200}}>주소</th>
        ...
        <tbody onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
          {branches.map((b,i) => (
            <tr key={b.id}
              ...
              <td draggable onDragStart={()=>setDragIdx(i)} onDragOver={e=>{e.preventDefault();setDragOverIdx(i)}} onDragEnd={handleDragEnd}
                style={{textAlign:"center",cursor:"grab",touchAction:"none"}}><I name="grip" size={14} color="#999"/></td>
...
function AdminSaleItems({ data, setData }) {
  ...
  const [editData, setEditData] = useState({});
  const [showAdd, setShowAdd] = useState(false);
  const [newSvc, setNewSvc] = useState({ cat:cats[0]?.id||"sc1", name:"", dur:20, priceF:0, priceM:0, note:"", isPackage:false, pkgCount:5, pkgPriceF:0, pkgPriceM:0 });
  ...
  const saveEdit = () => { 
    ...
    sb.update("services", editId, {cat:editData.cat, name:editData.name, dur:editData.dur, price_f:editData.priceF, price_m:editData.priceM, note:editData.note||"", sort:editData.sort||0, is_package:!!editData.isPackage, pkg_count:editData.pkgCount||0, pkg_price_f:editData.pkgPriceF||0, pkg_price_m:editData.pkgPriceM||0}).catch(console.error);
    setEditId(null); 
  };
  const deleteSvc = (id) => { 
  ...
  const addSvc = () => {
    ...
    const svc = {...newSvc, id:uid(), priceF:Number(newSvc.priceF)||0, priceM:Number(newSvc.priceM)||0, dur:Number(newSvc.dur)||20, sort:services.length, isPackage:!!newSvc.isPackage, pkgCount:Number(newSvc.pkgCount)||0, pkgPriceF:Number(newSvc.pkgPriceF)||0, pkgPriceM:Number(newSvc.pkgPriceM)||0};
    setServices(p=>[...p, svc]);
    sb.insert("services", {id:svc.id, business_id:_activeBizId, cat:svc.cat, name:svc.name, dur:svc.dur, price_f:svc.priceF, price_m:svc.priceM, note:svc.note||"", sort:svc.sort, is_package:svc.isPackage, pkg_count:svc.pkgCount, pkg_price_f:svc.pkgPriceF, pkg_price_m:svc.pkgPriceM}).catch(console.error);
    setNewSvc({ cat:cats[0]?.id||"sc1", name:"", dur:20, priceF:0, priceM:0, note:"", isPackage:false, pkgCount:5, pkgPriceF:0, pkgPriceM:0 }); setShowAdd(false);
  };
  ...
  const downloadXl = () => XL.download(services.map(s=>({카테고리:cats.find(c=>c.id===s.cat)?.name||"",시술명:s.name,여성가:s.priceF,남성가:s.priceM,소요분:s.dur,다회권:s.isPackage?"Y":"",회수:s.pkgCount||"",비고:s.note||""})),"시술목록.xlsx","시술",["카테고리","시술명","여성가","남성가","소요분","다회권","회수","비고"]);
  const uploadXl = async (e) => {
    const file = e.target.files[0]; if(!file) return;
    const rows = await XL.readFile(file);
    ...
    const newSvcs = rows.map((r,i)=>{
      ...
      return {id:uid(), cat:cat?.id||localCats[0]?.id||"sc1", name:String(r["시술명"]||r["name"]||"").trim(), priceF:Number(r["여성가"]||r["priceF"])||0, priceM:Number(r["남성가"]||r["priceM"])||0, dur:Number(r["소요분"]||r["dur"])||20, note:String(r["비고"]||"").trim(), sort:i, isPackage:String(r["다회권"]||"").toUpperCase()==="Y", pkgCount:Number(r["회수"])||0, pkgPriceF:0, pkgPriceM:0};
    }).filter(r=>r.name);
    ...
    if(newSvcs.length) {
      ...
      if (mode) {
        ...
        setServices(newSvcs);
        for (const s of newSvcs) { await sb.insert("services",{id:s.id, business_id:_activeBizId, cat:s.cat, name:s.name, dur:s.dur, price_f:s.priceF, price_m:s.priceM, note:s.note||"", sort:s.sort, is_package:!!s.isPackage, pkg_count:s.pkgCount||0, pkg_price_f:s.pkgPriceF||0, pkg_price_m:s.pkgPriceM||0}).catch(console.error); }
      } else {
        const maxSort = services.reduce((m,s)=>Math.max(m,s.sort||0),0);
        newSvcs.forEach((s,i) => s.sort = maxSort + 1 + i);
        setServices(p=>[...p,...newSvcs]);
        newSvcs.forEach(s=>sb.insert("services",{id:s.id, business_id:_activeBizId, cat:s.cat, name:s.name, dur:s.dur, price_f:s.priceF, price_m:s.priceM, note:s.note||"", sort:s.sort, is_package:!!s.isPackage, pkg_count:s.pkgCount||0, pkg_price_f:s.pkgPriceF||0, pkg_price_m:s.pkgPriceM||0}).catch(console.error));
      }
  ...
  return <div>
    ...
    {showAdd && <div className="card" style={{padding:14,marginBottom:14}}>
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
        ...
        <FLD label="여성가(1회)">{inp(newSvc.priceF, v=>setNewSvc(p=>({...p,priceF:v})), 90, "number")}</FLD>
        <FLD label="남성가(1회)">{inp(newSvc.priceM, v=>setNewSvc(p=>({...p,priceM:v})), 90, "number")}</FLD>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",marginTop:8}}>
        <label style={{fontSize:11,display:"flex",alignItems:"center",gap:4,cursor:"pointer"}}>
          <input type="checkbox" checked={newSvc.isPackage} onChange={e=>setNewSvc(p=>({...p,isPackage:e.target.checked}))} style={{accentColor:"#7c7cc8"}}/>
          <span style={{fontWeight:600,color:newSvc.isPackage?"#7c7cc8":"#999"}}>다회권</span>
        </label>
        {newSvc.isPackage && <>
          <FLD label="회수">{inp(newSvc.pkgCount, v=>setNewSvc(p=>({...p,pkgCount:v})), 50, "number")}</FLD>
          <FLD label="다회권 여성가">{inp(newSvc.pkgPriceF, v=>setNewSvc(p=>({...p,pkgPriceF:v})), 90, "number")}</FLD>
          <FLD label="다회권 남성가">{inp(newSvc.pkgPriceM, v=>setNewSvc(p=>({...p,pkgPriceM:v})), 90, "number")}</FLD>
        </>}
        <button className="btn-p btn-sm" onClick={addSvc} style={{padding:"6px 16px",marginLeft:"auto"}}>등록</button>
      </div>
    </div>}
    ...
    <div className="tw admin-tw" style={{maxHeight:"calc(100vh - 340px)",overflow:"auto"}}>
      <table><thead><tr>
        ...
        <th style={{width:50,textAlign:"center"}}>소요</th><th style={{width:100}}>비고</th><th style={{width:45,textAlign:"center",fontSize:9}}>다회권</th><th style={{width:75,textAlign:"center"}}>관리</th>
      </tr></thead><tbody>
        {filtered.map((svc,i)=>{
          ...
          if (isEdit) return <tr key={svc.id} ref={drag.rowRef(i)} style={{background:"#f5f5ff"}}>
            ...
            <td style={{textAlign:"center"}}><label style={{fontSize:9,display:"flex",alignItems:"center",gap:2}}><input type="checkbox" checked={!!editData.isPackage} onChange={e=>setEditData(p=>({...p,isPackage:e.target.checked}))}/>{editData.isPackage?editData.pkgCount+"회":""}</label>
            {editData.isPackage && <div style={{display:"flex",gap:2,marginTop:2}}>{inp(editData.pkgCount||5,v=>setEditData(p=>({...p,pkgCount:v})),30,"number")}<span style={{fontSize:8,color:"#999"}}>회</span></div>}</td>
          ...
          return <tr key={svc.id} ref={drag.rowRef(i)} draggable onDragStart={()=>drag.onDragStart(i)} onDragEnter={()=>drag.onDragEnter(i)} onDragEnd={drag.onDragEnd} onDragOver={e=>e.preventDefault()}
            ...
            <td style={{fontWeight:500,fontSize:12}}>{svc.name}{svc.isPackage && <span style={{marginLeft:4,fontSize:8,background:"#7c7cc8",color:"#fff",borderRadius:3,padding:"1px 4px",verticalAlign:"middle"}}>{svc.pkgCount}회권</span>}</td>
            ...
            <td style={{textAlign:"center"}}>{svc.isPackage ? <span style={{fontSize:9,background:"#7c7cc8",color:"#fff",borderRadius:3,padding:"1px 4px"}}>{svc.pkgCount}회</span> : <span style={{fontSize:9,color:"#ccc"}}>-</span>}</td>
...
function AdminResSources({ data, setData }) {
  const [sources, setSources] = useState(()=>[...(data?.resSources || [])].sort((a,b)=>(a.sort||0)-(b.sort||0)));
  useEffect(() => { setData(p => ({...p, resSources: sources})); }, [sources]);
  const [showAdd, setShowAdd] = useState(false);
  ...
  return <div>
    ...
    <div className="card" style={{padding:20,maxWidth:540}}>
      ...
      {[
        ...
        ["secretKey","Secret Key","API 보안 설정에서 생성"],
        ["senderKey","발신 프로필 키","카카오 채널 연동 후 발급"]
      ].map(([k,label,ph])=>
        <div key={k} style={{marginBottom:10}}>
          <div style={{fontSize:11,fontWeight:600,color:"#555",marginBottom:3}}>{label}</div>
          <input className="inp" type={k==="secretKey"?"password":"text"} value={cfg[k]||""} onChange={e=>up(k,e.target.value)} placeholder={ph} style={{width:"100%",padding:"7px 10px",fontSize:12}}/>
        </div>
      )}
      <div style={{display:"flex",gap:8,marginTop:14}}>
        <button className="btn-p btn-sm" onClick={save} style={{padding:"8px 20px"}}>{saved?"저장됨 ✓":"저장"}</button>
        <button className="btn-s btn-sm" onClick={testConnection} disabled={testing} style={{padding:"8px 14px",fontSize:11}}>{testing?"확인 중...":"연결 테스트"}</button>
      </div>
    ...
    <div className="card" style={{padding:20,maxWidth:540,marginTop:16}}>
      ...
      {[
        ...
        ["tplReminder","시술 후 리마인더","시술 완료 후 지정 간격으로 발송"],
        ["tplChange","예약 변경 알림","예약 수정/취소 시 발송"]
      ].map(([k,label,desc])=>
        <div key={k} style={{display:"flex",gap:8,alignItems:"center",marginBottom:8}}>
          <div style={{width:120}}>
            ...
            <div style={{fontSize:9,color:"#bbb"}}>{desc}</div>
          </div>
    ...
    <div className="card" style={{padding:20,maxWidth:540,marginTop:16}}>
      <div style={{fontSize:13,fontWeight:700,marginBottom:12,color:"#333"}}>자동 발송 설정</div>
      {[
        ["autoReservation","예약 등록 시 알림톡 자동 발송"],
        ["autoReminder","시술 완료 후 리마인더 발송"]
      ].map(([k,label])=>
        <label key={k} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,cursor:"pointer"}}>
          <input type="checkbox" checked={!!cfg[k]} onChange={e=>up(k,e.target.checked)} style={{accentColor:"#7c7cc8"}}/>
          <span style={{fontSize:12,color:"#555"}}>{label}</span>
        </label>
      )}
      {cfg.autoReminder && <div style={{display:"flex",gap:8,alignItems:"center",marginTop:4,marginLeft:26}}>
        <span style={{fontSize:11,color:"#888"}}>리마인더 간격:</span>
        <select className="inp" value={cfg.reminderDays||7} onChange={e=>up("reminderDays",Number(e.target.value))} style={{width:80,padding:"4px 6px",fontSize:11}}>
    ...
    <div className="card" style={{padding:20,maxWidth:540,marginTop:16}}>
      <div style={{fontSize:13,fontWeight:700,marginBottom:8,color:"#333"}}>설정 가이드</div>
    ...
    {!isListening && !loading && !result && <div style={{flexShrink:0,padding:"8px 12px 16px",borderTop:"1px solid #f0f0f0",background:"#fff"}}>
      <div style={{display:"flex",alignItems:"flex-end",gap:4,background:"#f0f0f5",borderRadius:24,padding:"6px 6px 6px 16px"}}>
        <textarea ref={inputRef} value={input} onChange={e=>{setInput(e.target.value);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,120)+"px";}}
          ...
          onPaste={handlePaste}
          placeholder="메시지를 입력하거나 이미지를 붙여넣으세요..." rows={1}
...
const S = {
  ...
  mobHdr: { padding:"10px 16px", background:"#fff", display:"flex", alignItems:"center", gap:12 },
...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media(max-width:768px){
    ...
    .tl-topbar{flex-direction:column!important;gap:4px!important;padding:6px 6px!important;align-items:stretch!important}
    ...
    .tl-days{width:100%;justify-content:space-between;padding:0 2px}
